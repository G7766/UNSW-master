...
agent_at(X0,Y0),
assert(goal(goal(X0,Y0))),
update_intent(Goal,Intentions,NewIntentions),
incorporate_goals(Goals, NewIntentions, Intentions1).


update_intent(Goal,intents(Int_drop,Int_pick),intents(Int_drop,Int_pick)):-
not(solve(Goal,_,_,_)),!.

update_intent(Goal,intents(Int_drop,Int_pick),intents(Int_drop,Int_pick1)):-
solve(Goal,_,G,_),
update_intent_g(Goal,G,Int_pick,Int_pick1),!.

update_intent_g(Goal,_,[],[[Goal,[]]]).
update_intent_g(Goal,_,[[Goal,Plan]|Tail],[[Goal,Plan]|Tail]).
update_intent_g(Goal,G,[[HGoal,HPlan]|Tail],[[HGoal,HPlan]|Tail1]):-
solve(HGoal,_,GHL,_),
G>=GHL,
update_intent_g(Goal,G,Tail,Tail1).
update_intent_g(Goal,G,[[HGoal,HPlan]|Tail],[[Goal,[]],[HGoal,HPlan]|Tail]):-
solve(HGoal,_,GHL,_),
G<GHL,!.
...
agent_at(X0,Y0),
assert(goal(goal(X0,Y0))),
get_action_pick(Int_pick,Int_pick1,Action),
retract(goal(goal(X0,Y0))),!.
...
initial_intentions2(X,Y,intents(L,[])):-
    ...
    assert(goal(goal(X0,Y0))),
    solve(goal(X,Y),Path,_,_),
    retract(goal(goal(X0,Y0))),writeln(Path),
    findall(A,(member(goal(X1,Y1),Path), land(X1,Y1), A = [goal(X1,Y1),[]]), L).
