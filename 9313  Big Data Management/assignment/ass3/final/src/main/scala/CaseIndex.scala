...
object CaseIndex {
  ...
  def processAllFile(path: List[File]):Unit= {
    ...
    path.foreach(file =>{
      ...
      //val addresss = "http://localhost:9200/legal_idx/cases/"+f_name+"?pretty"
      //println(addresss)
      val final_put = Http("http://localhost:9200/legal_idx/cases/"+f_name+"?pretty")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val mapping_request = Http("http://localhost:9200/legal_idx/cases/_mapping?pretty")
      .postData(post_data)
      .method("PUT")
      .header("Content-Type", "application/json")
      .option(HttpOptions.connTimeout(150000))
