...
for k in range(2):
    ...
    for i in range(8):
        ...
        baseline_err = np.mean(scores[0])
        for j in range(5):
            print(" | {:>6.2%} ({:>5.2%}) {}" .format(np.mean(scores[j + offset[k]]),
                                                      np.std(scores[j + offset[k]]),
                                                      '*' if p_value[j] < 0.05 else ' '), end = '')
            if j==0:
                percent10_err = np.mean(scores[j + offset[k]])
                calculate = (baseline_err -percent10_err)/baseline_err
                a10 = a10 + calculate
            if j==4:
                percent100_err = np.mean(scores[j + offset[k]])
                calculate = (baseline_err - percent100_err) / baseline_err
                a100 = a100 + calculate
        print()
    #baseline_err = baseline_err/8
    #percent10_err = percent10_err/8
    #percent100_err = percent100_err/8
    #print('baseline_err: ',baseline_err)
    #print('percent10_err: ', percent10_err)
    #print('percent100_err: ', percent100_err)
    print('after 10: ',a10/8)#(baseline_err - percent10_err)/(baseline_err) * 100)
    print('after 100: ',a100/8)#(baseline_err - percent100_err)/(baseline_err) * 100)
    print()
