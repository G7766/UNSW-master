...
class dataOperation():
    ...
    # insert / delete / query data in mongodb
...
class import_data_Operation(Resource):
    ...
    @api.doc(description="Add a new Data")
    def post(self,collections):
        #col_id =  collections["collection_id"]
        ...
        return_record['indicator_id'] = indicator_id

        return return_record

    @api.response(404, 'Data was not found')
    @api.response(200, 'Successful')
    @api.doc(description="Get Data list")
...
class year_country(Resource):
    @api.response(404, 'Data was not found')
    @api.response(200, 'Successful')
    @api.doc(description="Get Data list by year and country")
...
class Retrive_top_bottom(Resource):
    @api.response(404, 'Data was not found')
    @api.response(200, 'Successful')
    @api.doc(description="Retrieve top/bottom economic indicator values for a given y")
    def get(self, collections,collection_id,year,query):
        user = mongo.db[collections]
        if query[:3] == 'top':
            query = 'top'
            number = int(query[3:])
            if number<1 or number>100:
                api.abort(404, "The query should be in 1-100")
        if query[:6] == 'bottom':
            query = 'bottom'
            number = int(query[6:])
            if number<1 or number>100:
        ...
        for x in user.find():
            if collection_id == str(x['_id']):
                ...
                length = len(record)
                if length == 0:
                    api.abort(404, "The data is not exit!")
...
if __name__ == '__main__':
    #id = "NY.GDP.MKTP.CD"
    #datalist = get_data()
    #collection_data = get_target_data(datalist,id)
