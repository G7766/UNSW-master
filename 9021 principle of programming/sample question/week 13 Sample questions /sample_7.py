...
def find_word_horizontally(grid, word):
    ...
    for i in range(0,len(grid)):
        for j in range(len(grid[i])-lenth+1):
            if grid[i][j:j+lenth]==l:
                return i+1,j+1
...
def find_word_vertically(grid, word):
    ...
    for j in range(0,len(grid[0])):
        for i in range(0,len(grid)-lenth+1):
            m=[]
...
def find_word_diagonally(grid, word):
    l=list(word)
    lenth=len(l)
    # up-left top to down-right
    for i in range(len(grid)-lenth+1):
        for j in range(len(grid[i])-lenth+1):
            m=[]
            for k in range(lenth):
                m.append(grid[i+k][j+k])
            #print(m)
            if m==l:
                return i+1, j+1
    # Replace pass above with your code
