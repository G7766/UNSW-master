...
remainders_modulo_4 = [0] * 4
for i in range(nb_of_elements):
	#if remainders_modulo_4[L[i] /5] += 1
	if L2[i]<5:
		remainders_modulo_4[0]+= 1
	elif L2[i]>=5 and L2[i]<10:
		remainders_modulo_4[1]+= 1
	elif L2[i]>=10 and L2[i]<15:
		remainders_modulo_4[2]+= 1
	elif L2[i]>=15 and L2[i]<20:
...
for i in range(4):
    if remainders_modulo_4[i] == 0:
        ...
        print(f'There are {remainders_modulo_4[i]} elements between', end = ' ')
    print(f' {i*5} and {(i+1)*5-1}.')
